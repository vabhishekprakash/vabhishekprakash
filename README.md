...
- **[rag-eval-system](https://github.com/vabhishekprakash/rag-eval-system)**: a fully local RAG pipeline (FAISS, Mistral 7B via Ollama) benchmarked against a 28-question set I wrote over the OCPP specification. Both RAGAs metrics ranked a no-retrieval baseline above every retrieval configuration, while blind grading found that baseline correct on 0 of 23 answerable questions.
- **[Bail_Reckoner](https://github.com/vabhishekprakash/Bail_Reckoner)**: computes an undertrial prisoner's statutory entitlement to release under Section 479 of the BNSS, 2023. It walks six statutory gates in order and cites the provision behind each step, and its offence data is going through review before any result is treated as verified.
